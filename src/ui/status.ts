...
import { accent, danger, faint, glyph, muted, text, warn } from "./theme.ts";
...
const FOOTER_SEPARATOR = ` ${glyph.divider} `;
 ...
 * An empty result means the row can be hidden.
 */
export function buildActivityText(
	...
	if (snapshot.busy) {
		segments.push({ text: `${options.spinnerFrame} ${snapshot.status || "Working"}`, tone: warn });
	...
	for (const segment of segments) {
		if (chunks.length === 0) {
			const first = truncate(segment.text, options.width);
			if (first.length === 0) break;
			chunks.push(segment.tone(first));
			used = first.length;
			continue;
		}
		const cost = ACTIVITY_SEPARATOR.length + segment.text.length;
		if (used + cost > options.width) continue;
		chunks.push(faint(ACTIVITY_SEPARATOR), segment.tone(segment.text));
		used += cost;
	}
 ...
 * `cwd │ remote │ provider/model │ thinking level │ used/limit (pct) │ cache pct`.
...
export function buildFooterText(
	snapshot: AppSnapshot,
	options: { width: number; home: string; expanded: boolean },
): StyledText {
	...
	segments.push({ text: model, tone: accent, rank: 0, fit: (width) => truncate(model, width) });
	...
	if (options.expanded) segments.push({ text: "tool output expanded", tone: accent, rank: 4 });

	const widths = segments.map((segment) =>
		segment.fit ? Math.min(segment.text.length, ELASTIC_MIN) : segment.text.length,
	);
